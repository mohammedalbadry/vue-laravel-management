<template>
    <div>
        <div class="row">
            ...
            <div class="col-md-5">
                <div class="invoice">
                    <div v-if="newDataErrors.clint_id" class="text-danger myerror">
                            clint must be selected
                    </div>
                    <div class="clint">
                        
                        <template v-if="clintInfo == null">
                            <button class="btn btn-block btn-sm btn-info" data-toggle="modal" data-target="#clint_search">get clint</button>
                        </template>
                        <template v-if="clintInfo !== null">
                    ...
                    <table v-if="invoiceItems.length !== 0" class="table mb-0">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in invoiceItems" :key="index">
                                ...
                                <td>$ {{item.price}}</td>
                                <td>
                                    <button @click="removeFromOrderList(item.id)" class="btn btn-sm btn-danger"><i class="fa fa-trash-alt"></i></button>
                                </td>
                            </tr>
                    ...
                    <div class="row"  v-if="invoiceItems.length !== 0 && clintInfo !== null">
                        <div class="col-12">
                            <button @click="confirmOrder()" class="btn btn-block btn-success mb-3">confirm</button>
                        </div>
                        <div class="col-6">
        ...
        <div class="all_modals">
            <!-- search Modal -->
            <div class="modal fade" id="clint_search" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                <div class="modal-dialog  modal-lg modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLongTitle">search clint</h5>
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <input @input="searchClint()" v-model="clint_search"  type="text" class="form-control" placeholder="enter clint name or phone">
                            </div> 

                            <div class="clints_container">
                                <div class="row">
                                    <div v-for="(item, index) in all_clint_search" :key="index" class="col-6 col-sm-4 col-md-3">
                                        <div @click="setClintInfo(item)" class="clint_item mb-3">
                                            <h5>{{item.name}}</h5>
                                            <h6>{{item.phone}}</h6>
                                            <h6>{{item.address}}</h6>
                                        </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <router-link to="/orders" class="btn btn-sm btn-success" target="_blank">
                                create new clint
                            </router-link>
                        </div>
                    </div>
...
export default {
    ...
    methods:{
        ...
        searchClint(){
            ...
            var fd = new FormData()
            fd.append("clint", this.clint_search);
            ...
            .then(response=> {
                this.all_clint_search = response.data.data
            })
            .catch( error => {
               //console.log(error)
            });
        ...
        setClintInfo(item){
            ...
            this.all_clint_search = null
            this.clint_search = null
        },
        clear(){
            this.clintInfo = null
            this.orderItem = []
        ...
        confirmOrder: function(){
            ...
            fd.append('clint_id', this.clintInfo.id)
